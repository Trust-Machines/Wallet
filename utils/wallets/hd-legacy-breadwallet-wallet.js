...
export class HDLegacyBreadwalletWallet extends HDLegacyP2PKHWallet {
  ...
  _calcNodeAddressByIndex(node, index, p2wpkh = false) {
    ...
    if (node === 0) {
      _node =
        this._node0 ||
        (this._node0 = bip32.fromBase58(this.getXpub()).derive(node));
    }
    if (node === 1) {
      _node =
        this._node1 ||
        (this._node1 = bip32.fromBase58(this.getXpub()).derive(node));
    }
    const pubkey = _node.derive(index).publicKey;
    const address = p2wpkh
      ? bitcoinjs.payments.p2wpkh({ pubkey }).address
  ...
  _getNodeAddressByIndex(node, index) {
    ...
    if (
      (node === 0 &&
        this._external_segwit_index !== null &&
        index >= this._external_segwit_index) ||
      (node === 1 &&
        this._internal_segwit_index !== null &&
        index >= this._internal_segwit_index)
    ) {
      p2wpkh = true;
    }
  ...
  async fetchBalance() {
    try {
      if (
        ...
        const [nextFreeExternalLegacy, nextFreeInternalLegacy] =
          await Promise.all([
            this._binarySearchIteration(0, 1000, 0, false),
            this._binarySearchIteration(0, 1000, 1, false),
          ]);
        const [nextFreeExternalBech32, nextFreeInternalBech32] =
          await Promise.all([
            this._binarySearchIteration(
              nextFreeExternalLegacy,
              nextFreeExternalLegacy + 1000,
              0,
              true
            ),
            this._binarySearchIteration(
              nextFreeInternalLegacy,
              nextFreeInternalLegacy + 1000,
              1,
              true
            ),
          ]);
  ...
  _addPsbtInput(psbt, input, sequence, masterFingerprintBuffer) {
    ...
    const ProxyClass = input.address.startsWith("bc1")
      ? AbstractHDElectrumWallet
      : HDLegacyP2PKHWallet;
    const proxy = new ProxyClass();
    return proxy._addPsbtInput.apply(this, [
      psbt,
      input,
      sequence,
