...
export class MultisigHDWallet extends AbstractHDElectrumWallet {
  ...
  getSecret(coordinationSetup = false) {
    let ret = '# TM Wallet Multisig setup file\n';
    if (coordinationSetup)
      ret += '# this file contains only public keys and is safe to\n# distribute among cosigners\n';
  ...
  howManySignaturesCanWeMake() {
    ...
    for (const cosigner of this._cosigners) {
      if (!MultisigHDWallet.isXpubString(cosigner) && !MultisigHDWallet.isXprvString(cosigner))
  ...
  createTransaction(
    ...
    for (const u of utxos) {
      ...
      if (this.isNativeSegwit()) {
        u.script.length =
          u.script.length || Math.ceil((8 + this.getM() * 74 + this.getN() * 34) / 4);
      } else if (this.isWrappedSegwit()) {
        u.script.length =
          u.script.length || 35 + Math.ceil((8 + this.getM() * 74 + this.getN() * 34) / 4);
      } else {
